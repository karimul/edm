...
def load_or_generate_inception_embedding(directory, cache_dir, inception_path, ref=False):
    ...
    imgs = None
    if ref is True:
        with dnnlib.util.open_url(directory) as f:
            imgs = np.load(f)
    else:
...
if __name__ == '__main__':
    ...
    real_embeddings = load_or_generate_inception_embedding(
        reference_dir, args.cache_dir, args.inception_path, True)
    prd_data = []
